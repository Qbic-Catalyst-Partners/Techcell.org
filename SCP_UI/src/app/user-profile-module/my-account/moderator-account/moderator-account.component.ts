...
@Component({
  ...
  styleUrl: './moderator-account.component.scss',

  providers: [{ provide: MAT_DATE_FORMATS, useClass: MyFormat }],
})
export class ModeratorAccountComponent implements OnInit, OnDestroy {
  public profilePhoto: any = '';
  public profilePhotoName: any = '';
  /** Holds byte[] that will be sent to backend */
  ...
  public userInfoForm: any = this._fb.group({
    ...
    domainExpertise: ['', Validators.required],
    photo: [''],
    aboutMe: [''],
  });
    ...
    this.userProfileService.getUserDetail().subscribe({
      next: (res) => {
        this.userDetails = {
          ...
          dob: res?.data?.userDetailResponseDTO.dob
            ? new DatePipe('en-US').transform(
                res?.data?.userDetailResponseDTO.dob.toString(),
                'yyyy-MM-dd'
              )
            : null,
        };
  ...
  public async upload(file: any, from: any) {
    ...
    this.userProfileService.updateProfilePhoto({ profilePhoto: this.profilePhotoBytes }).subscribe({
      next: () => {
        ...
        this.userInfoForm.get('photo')?.updateValueAndValidity();
        // Refresh details to get image from backend
  ...
  public submitUserInfoForm(): void {
    ...
    if (this.userInfoForm.valid) {
      let payload = {
        ...
        profilePhoto: this.profilePhotoName ? this.profilePhotoBytes : '',
        programName: '',
        ...
        stream: '',
        workExp: this.userInfoForm.controls['workExperience'].value,
        qualification: this.userInfoForm.controls['Qualification'].value,
        domailExp: this.userInfoForm.controls['domainExpertise'].value,
        ...
        designation: this.userInfoForm.controls['currentDesignation'].value,
  ...
  patchUserInfo(formValue: any) {
    this.userInfoForm.patchValue({
      ...
      domainExpertise: formValue?.domailExp,
      currentCompany: formValue?.currentCompany,
      currentDesignation: formValue?.designation,
      workExperience: formValue?.workExp,
