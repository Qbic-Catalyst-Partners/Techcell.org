...
export class JobListComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public header_data: any[] = [
    ...
    },
    {
      ...
      isSort: true,
      sort: true,
      sortFieldName: 'designation',
    },
    {
      fieldName: 'Job Type',
      isSort: true,
      sort: true,
      sortFieldName: 'jobType',
    },
  ...
  /** Flag to ensure single initial fetch */
  ...
  ngOnInit(): void {
    ...
    // If ngOnChanges did not fetch data (component has no inputs), load once.
    if (!this.dataFetched) {
    ...
    // Subscribe to real-time job additions
  ...
  private loadData(payload: any): void {
    this.careerService.getListJobs(payload).subscribe((res: any) => {
      this.isdata = res?.data && res.data.length == 0 ? true : false;
      let filteredData = res.data;
      if (this.userInfo.role !== 'Admin') {
        filteredData = filteredData.filter((job:any)=> job.status==='Active');
      }
      if (this.showOnlyApplied && this.userInfo.role === 'Student') {
        filteredData = filteredData.filter((job:any)=> job.applied);
      }
  ...
  toggleAction(row: any) {
    this.activeTooltipId = row.jobId;
  }
  ...
  sortHeader(column: any) {
    ...
    if (
      ...
      this.careerService.getListJobs(payload).subscribe({
        next: (res: any) => {
  ...
  openModal(item: any, type: string, count: number) {
    if (count) {
      const modalRef = this.modalService.open(JoblistingRecivedComponent, {
        backdrop: 'static',
        keyboard: true,
        size: 'lg',
        centered: true,
  ...
  public loadSelectedTagData(): void {
    this.jobService.getJobsByTagIdByID({ id: this.tag }, this.page).subscribe({
      next: (res: any) => {
        this.isdata = res?.data && res.data.length == 0 ? true : false;
        let filteredData = res.data;
        if (this.userInfo.role !== 'Admin') {
        ...
        if (filteredData.length > 0) {
          ...
          this.commonService.dialog({
            type: 'newErrorModal',
            header: 'No content',
            // message1: `No job listings found for ${tagName}`,
