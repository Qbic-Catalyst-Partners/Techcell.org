...
import { CertificationListingModalComponent } from './certification-listing-modal/certification-listing-modal.component';
import { AuthUtils } from '../../shared/utility/auth-utils';
import { ApiService } from '../../shared/services/api.service';
import { ADMIN_ROLE, CORPORATE_ROLE } from '../../common/constants';

@Component({
  selector: 'app-certification-grid-view',
  templateUrl: './certification-grid-view.component.html',
  styleUrl: './certification-grid-view.component.scss',
})
export class CertificationGridViewComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.isApiRefreshed?.subscribe((checkStatus) => {
      if (checkStatus) {
        if (this.selectedTagData?.tab === "CERTIFICATION") {
  ...
  public loadData(): void {
    forkJoin(
      this.tagListData.map((item: any) =>
        this.jobService.getCertificationByTagIdByID(item, 0)
      )
    ).subscribe((response: any) => {
      // Filter out inactive certifications for non-admin users
      this.hashTagListItems = response.map((tagGroup: any) => {
        if (tagGroup.data) {
          tagGroup.data = tagGroup.data.filter((cert: any) => {
            return this.userInfo?.role === 'Admin' || cert.status === 'Active';
          });
        }
        return tagGroup;
      });
    ...
    modalRef.result.then((response) => {
      if (response) {
        ...
        this.hashTagListItems.forEach((tagGroup: any) => {
          if (tagGroup.data) {
            tagGroup.data.forEach((cert: any) => {
              if (cert.certificationId === item.certificationId) {
                cert.applied = response;
              }
            });
          }
        });
        ...
        this.dynamicTagDataList.forEach((cert: any) => {
          if (cert.certificationId === item.certificationId) {
            cert.applied = response;
          }
        });
  ...
  nextSlide(index: any, item: any) {
    ...
    if (item.data && item.data.length > item.itemIndex) {
      ...
      this.jobService
        .getCertificationByTagIdByID({ id: item.tagId }, item.pageCount)
        .subscribe({
          next: (res) => {
            response = res.data;
  ...
  public loadSelectedTagData(tagId: any): void {
    console.log('Certification: loadSelectedTagData called with tagId', tagId, 'selectedTagData:', this.selectedTagData);
    ...
    this.jobService
      .getCertificationByTagIdByID(this.selectedTagData.selectedTag, 0)
      .subscribe({
        next: (res: any) => {
          if (res.data.length > 0) {
            this.dynamicTagDataList = res.data;
          } else {
            // Show unified "No content" modal when no certifications match the selected tag
