...
import { InternshipModalComponent } from './modals/internship-modal.component';
...
export class InternshipsComponent implements OnInit, OnChanges {
  ...
  @Input() isApiRefreshed: EventEmitter<boolean> = new EventEmitter<boolean>();
  ...
  ngOnInit(): void {
    ...
    this.isApiRefreshed.subscribe((checkStatus) => {
      if (checkStatus) {
        if (this.selectedTagData?.tab === 'INTERNSHIP') {
  ...
  public loadData(): void {
    forkJoin(
      this.tagListData.map((item: any) =>
        this.careerService.getInternshipByID(item, 0)
      )
    ...
    modalRef.result.then((response) => {
      if (response) {
        ...
        this.hashTagListItems.forEach((tagGroup: any) => {
          if (tagGroup.data) {
            tagGroup.data.forEach((internship: any) => {
              if (internship.internshipId === item.internshipId) {
                internship.applied = response;
              }
            });
          }
        });

        this.dynamicTagDataList.forEach((internship: any) => {
          if (internship.internshipId === item.internshipId) {
            internship.applied = response;
          }
        });
  ...
  nextSlide(index: any, item: any) {
    ...
    if (item.data && item.data.length > item.itemIndex) {
      ...
      this.careerService
        .getInternshipByID({ id: item.tagId }, item.pageCount)
        .subscribe({
          next: (res) => {
            response = res.data;
  ...
  public loadSelectedTagData(tagId: any): void {
    console.log('Internships: loadSelectedTagData called with tagId', tagId, 'selectedTagData:', this.selectedTagData);
    // If clicking the same tag again, deselect it
    if (this.currentlySelectedTagId === tagId) {
      this.dynamicTagDataList = [];
      this.currentlySelectedTagId = null;
      // restore previously removed carousel
      ...
      // Restore original data (if other mutations)
      return;
    }
    ...
    // Remove carousel group corresponding to new tag, first restore previous if any
    if (this.removedHashTagCarousel) {
      const { item, index } = this.removedHashTagCarousel;
      const insertIdx = index <= this.hashTagListItems.length ? index : this.hashTagListItems.length;
      this.hashTagListItems.splice(insertIdx, 0, item);
    ...
    this.careerService
      .getInternshipByID(this.selectedTagData.selectedTag, 0)
      .subscribe({
        next: (res: any) => {
          if (res.data.length > 0) {
            this.dynamicTagDataList = res.data;
          } else {
            // Show unified "No content" modal when no internships match the selected tag
            const tagDesc = this.selectedTagData?.selectedTag?.description || 'this criteria';
            this.commonService.dialog(
              'newErrorModal',
              // `No content found for ${tagDesc}`,
              'Stay tuned... We are bringing exciting opportunities for you!',
              '',
              'OK',
              'No content'
            );
